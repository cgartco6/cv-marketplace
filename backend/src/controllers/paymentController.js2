...
const { stripe, paypalClient, payfast, ozow, eftSimulation } = require('../config/payments');
...
exports.createCheckout = async (req, res) => {
  try {
    ...
    switch (paymentMethod) {
      ...
      case 'eft':
        const reference = eftSimulation.createReference(userId, order._id);
        paymentUrl = null;
        transactionId = reference;
        order.paymentMetadata = { eftReference: reference, bankDetails: eftSimulation.bankAccount };
        await order.save();
        break;
    }
    ...
    res.json({ orderId: order._id, paymentUrl, eftDetails: paymentMethod === 'eft' ? order.paymentMetadata : null });
  } catch (error) {
    console.error(error);
...
exports.stripeWebhook = async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    await finalizeOrder(orderId, 'stripe', session.id);
  }
...
exports.payfastWebhook = async (req, res) => {
  ...
  if (payment_status === 'COMPLETE') {
    await finalizeOrder(m_payment_id, 'payfast', req.body.pf_payment_id);
  }
  res.send('OK');
};
...
exports.confirmEFT = async (req, res) => {
  ...
  if (!order || order.paymentMethod !== 'eft') return res.status(404).json({ error: 'Invalid order' });
  // In production, verify via bank API
...
async function upgradeUserTier(userId, tier, billingPeriod) {
  ...
  if (billingPeriod === 'monthly') {
    ...
    user.tierExpiry = null; // one-time purchase, no expiry
  }
  await user.save();
}
