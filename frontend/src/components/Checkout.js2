...
export default function Checkout() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      const res = await axios.post('/api/payment/create-checkout', {
        tier, billingPeriod: billing, paymentMethod: method,
  ...
  return (
    <div className="max-w-md mx-auto bg-white p-6 rounded shadow">
      ...
      <button onClick={handleCheckout} disabled={loading} className="w-full bg-green-600 text-white py-2 rounded">
        {loading ? 'Processing...' : 'Pay Now'}
      </button>
      
      {eftDetails && (
        <div className="mt-4 p-3 bg-gray-100 rounded">
          <p>Please transfer R{eftDetails.amount} to:</p>
          <p>Bank: {eftDetails.bankDetails.accountName}</p>
          <p>Account: {eftDetails.bankDetails.accountNumber}</p>
          <p>Reference: {eftDetails.eftReference}</p>
          <p className="text-sm">After transfer, your account will be upgraded automatically within 24h.</p>
        </div>
      )}
    </div>
  );
